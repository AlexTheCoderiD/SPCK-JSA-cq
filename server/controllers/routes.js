...
async function addToCart(req, res) {
  try {
    ...
    jwt.verify(hashedUserId, jwtSecret, {}, async (err, user) => {
      ...
      if (user.id === owner) {
        return res.status(403).json({message:"You can't by your OWN course!"})
      }
      const newUser = await User.findByIdAndUpdate(
        user.id,
...
async function buyCourse(req, res) {
  try {
    ...
    // find resources
    const user = await User.findById(await verifyCookie(req.cookies));
    const { id: courseId } = req.params;
    ...
    if (!courseOwner) {
      return res.status(404).json({ message: "Course owner not found" });
    }

    // add boughtCourse and transfer money
    if (user.money >= course.cost.amount) {
      user.boughtCourse = [...user.boughtCourse, courseId];
      ...
      return res
        .status(402)
        .json({ messsage: "You do NOT have enough money for this course!" });
    }
  } catch (err) {
    res
      .status(500)
